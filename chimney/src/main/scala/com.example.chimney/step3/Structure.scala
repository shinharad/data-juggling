...
final case class Person(name: Person.Name, age: Person.Age, phone: Phone, address: Address)
case object Person {
  final case class Name(value: String) extends AnyVal {
    final override def productPrefix: String =
      s"${Person}.${Name}"
  ...
  final case class Raw(name: String, age: String, phone: String, address: Address.Raw) {
    final override def productPrefix: String =
      s"${Person}.${Raw}"
  }
}